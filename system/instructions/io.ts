...
export class Input implements Executable {
  run(instruction: Instruction) {
    const portNumber = instruction.operand;
    if (portNumber != 0) {
      throw new ReferenceError(
        `Port number ${portNumber} is not an input port.`,
      );
    }
...
export class Output implements Executable {
  run(instruction: Instruction) {
    const portNumber = instruction.operand;

    if (portNumber != 1) {
      throw new ReferenceError(
        `Port number ${portNumber} is not an output port.`,
      );
    }
    ...
    if (tosValue < 0) {
      throw new ReferenceError(
        `The value "${tosValue}" is not suitable for the output port.`,
      );
    }
